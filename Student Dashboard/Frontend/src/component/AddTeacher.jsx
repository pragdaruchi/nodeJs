...
export default function AddTeacher() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    await axios.post("http://localhost:1004/postData", formdata).then((res) => {
      alert(res.data.msg)
      ...
      <div className="flex-1 flex items-center justify-center p-6 relative z-10">
        <div className="w-full max-w-4xl grid md:grid-cols-2 bg-white/90 backdrop-blur-xl rounded-[28px] shadow-xl border border-white overflow-hidden">
          <div className="hidden md:flex flex-col justify-center p-10 bg-gradient-to-br from-emerald-400 to-green-400 text-white">
            <h2 className="text-3xl font-bold leading-tight">Manage <br /> Teachers</h2>
            <p className="mt-4 text-green-50 text-sm leading-relaxed max-w-xs"> Add and organize teacher information easily. </p>
          ...
          <div className="p-10 bg-white">
            <div className="mb-8">
              <h2 className="text-2xl font-semibold text-gray-800">Teacher Registration</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <button type="submit" className="w-full bg-gradient-to-r from-emerald-400 to-green-400 hover:from-green-500 hover:to-emerald-500 text-white py-3 rounded-xl font-semibold shadow-md hover:shadow-lg transition active:scale-95" >Register Teacher</button>
            </form>
          </div>
        </div>
      </div>
